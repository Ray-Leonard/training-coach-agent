...
Each meal is stored in `data/nutrition/menu/{meal-name}.md` and follows this format:
...
2. For an update or deletion, read only `data/nutrition/menu/{meal-name}.md`
...
For an add, create `data/nutrition/menu/{meal-name}.md`. For an update, edit that file. For a deletion, delete that file after explicit user confirmation.
...
1. Confirm `data/nutrition/menu/{meal-name}.md` was created, edited, or deleted as requested
...
- ✅ **Meal file**: `data/nutrition/menu/{meal-name}.md` updated successfully
...
6. User provides image → `skills/nutrition-database-management/modules/food-ingestion.md` adds eggs
7. All ingredients validated ✅
8. Creates `data/nutrition/menu/egg-sandwich.md`
9. "✅ Added 'Egg Sandwich' to menu with: multigrain-european-style-sliced-bread (2 slices), butter-salted (10g), eggs (2)"
