...
Manages `data/nutrition/MENU.md` — the user's meal templates. Ensures all ingredients exist in the food database before adding them to the menu.
...
SKILL_MODULE = skills/nutrition-database-management/modules/food-image-processor.md
```

## Menu Format

Each meal in `data/nutrition/MENU.md` follows this format:
...
1. Read `data/nutrition/MENU.md` to understand current structure
2. Note the format conventions (ingredient names, gram amounts, sections)
3. If `data/nutrition/MENU.md` doesn't exist → create it with a header
...
1. Search `data/nutrition/all_food_names.md` for a match (fuzzy name matching)
...
- → Then re-check `data/nutrition/all_food_names.md`
...
- Use ingredient names that **exactly match** `data/nutrition/all_food_names.md` entries (for reliable lookup later)
- Maintain consistent formatting
- Keep meals organized (alphabetically or by category)

### Step 5: Verification

After updating `data/nutrition/MENU.md`:
1. Confirm the meal entry exists in `data/nutrition/MENU.md` with correct format
2. Confirm all ingredient names match `data/nutrition/all_food_names.md` exactly
3. Confirm gram amounts are correct

> **Note**: `data/nutrition/MENU.md` is maintained directly and requires no additional generated artifact.
...
- ✅ **data/nutrition/MENU.md**: updated successfully
...
```bash
git add data/nutrition/MENU.md data/nutrition/all_food_names.md data/nutrition/individual_food_data/ data/nutrition/source_images/
git commit -m "feat(menu): {action} meal {meal_name}

- {Add/Update/Delete} meal in data/nutrition/MENU.md
- Validated {N} ingredients against data/nutrition/all_food_names.md
...
When matching user ingredient names to `data/nutrition/all_food_names.md`:
...
Before updating `data/nutrition/MENU.md`, confirm:
- [ ] All ingredients found in `data/nutrition/all_food_names.md`
...
**Agent**:
1. [Reads `data/nutrition/MENU.md` and `data/nutrition/all_food_names.md`]
...
8. Updates `data/nutrition/MENU.md`
